...
import { getAllReserve } from '../../redux/reserve/reserve';
...
export default function Reservation() {
  ...
  useEffect(() => {
    ...
    if (exist === 0) {
      ...
      const ReservedValues = reservation.filter((m) => m.user_id === local.id);
      console.log(ReservedValues);
      setValue(ReservedValues);
    }
  }, []);

  // const submitHandler = (id) => dispatch(deleteReserve(id));
  return (
        ...
        {reservation.map((reservation) => (
          <div key={reservation.id} className="flex flex-col bg-white border border-gray-200 rounded-lg shadow-md hover:scale-[102%] transition duration-200 ease-in-out">
            <div className="relative block shrink-0">
              ...
              // onClick={submitHandler(reservation.id)}
              >
